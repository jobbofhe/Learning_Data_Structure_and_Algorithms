 ...
 * 双链表队列与双链表栈的异同
 * 链表的方法相同
 * 不同的地方是 栈结构，push() 函数直接将结点插入头结点后即可；队列，则是将新节点查到尾节点的后边
 */
...
node *create_double_list_fifo()
{
    return create_double_list();
}

int destroy_double_list_fifo(node *plist)
{
...
{
    return append_value_to_tail(plist, pvalue);
}
...
int main(int argc, char const *argv[])
{
    ...
    node *pfifo = create_double_list_fifo();
    ...
    while(!fifo_is_empty())
    {
        people * info = pop(pfifo);
        print(info);
    }
    
    destroy_double_list(pfifo);
